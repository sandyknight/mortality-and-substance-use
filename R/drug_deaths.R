...
p1 + 
  geom_segment(x = 2023.4, y = 0, yend = 3250, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) + 
  annotate("text", x = 2023.5, y = 1668, label = "Drug poisoning deaths related to drug misuse,\nas classified in ONS data", hjust = 0) +
  geom_segment(x = 2023.4, y = 3336, yend = 3250 + 936, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) +
  annotate("text", x = 2023.5, y = 3250 + (936/2), label = "Drug poisoning deaths in drug treatment or within a\nyear of leaving treatment but not classified as related to\ndrug misuse in ONS data", hjust = 0) +
  geom_segment(x = 2023.4, y = 3336 + 930, yend = 3250 + 936 + 1386, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) +
  annotate("text", x = 2023.5, y = 3250 + 936 + (1386/2), label = "Deaths in treatment with a cause other than poisoning", hjust = 0) +
  geom_segment(x = 2023.4, y = 3336 + 930 + 1380, yend = 3250 + 936 + 1386 + 541, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) +
  annotate("text", x = 2023.5, y = 3250 + 936 + 1386 + (541/2), label = "Deaths within a year of leaving treatment with a cause\nother than poisoning", hjust = 0) +
  geom_segment(x = 2023.4, y = 3336 + 930 + 1380 + 541, yend = 3250 + 936 + 1386 + 541 + 2808, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm")), colour = "darkgrey") +
  annotate("text", x = 2023.5, y = 3250 + 936 + 1386 + 541 + (2808/2), label = "Deaths a year more after leaving treatment with a cause\nother than poisoning", hjust = 0, colour = "darkgrey") +
  scale_alpha_manual(values = c("Initial poisoning deaths" = 1,
  ...
  scale_linetype_manual(values = c("Initial poisoning deaths" = 1,
                                "Additional poisoning deaths" = 1,
                                "Non-poisoning deaths: Died in treatment" = 1,
                                "Non-poisoning deaths: Died within a year of discharge" = 1,
                                "Non-poisoning deaths: Died one or more years following discharge" = 3)) +
  geom_segment(x = 2023, xend = 2020.75, y = 6199, arrow = arrow(length = unit(.3, "cm")))
